...
class ODMMvsTexStage(types.ODM_Stage):
    def process(self, args, outputs):
        ...
        if args.flir:
            log.ODM_INFO("Entered Flir in texturing. Changing Folders")
            #these first two lines might not be needed because we will only be using
            #undistorted images from this point on
            os.system('mv ' + str(args.project_path) + '/images/* ' + str(args.project_path) + '/images_backup') #images_raw files para backup folder
            os.system('mv ' + str(args.project_path) + '/flir_output/* ' + str(args.project_path)+'/images')#extractor_images files para images folder
            os.system('mkdir ' + str(os.path.join(tree.opensfm,"undistorted")) + '/original_images')
            os.system('mv ' + str(os.path.join(tree.opensfm,"undistorted")) + '/images/* ' + str(os.path.join(tree.opensfm,"undistorted")) + '/original_images')#undistorted_data_raw files para backup folder
            os.system('mv ' + str(os.path.join(tree.opensfm,"undistorted")) + '/images_backup/* ' + str(os.path.join(tree.opensfm,"undistorted")) + '/images')#undistorted_extractor_images files para images folder

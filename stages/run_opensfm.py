...
class ODMOpenSfMStage(types.ODM_Stage):
    def process(self, args, outputs):
        ...
        if args.flir:
            log.ODM_INFO("Entered in Flir. Changing Folders")
            os.system('mkdir ' +str(args.project_path) + '/images_backup')
            os.system('mv ' + str(args.project_path) + '/images/* ' + str(args.project_path) + '/images_backup') #image_raw files into images_backup folder
            #os.system('rm -r ' + str(args.project_path) + '/images')
            os.system('mkdir '+ str(args.project_path) + '/images')
            os.system('mv ' + str(args.project_path) + '/flir_output/* ' + str(args.project_path)+'/images') #extractor_images files into images folder
        ...
        if args.flir:
            #os.system('mkdir ' + str(args.project_path) + '/images_2/');
            #os.system('cp ' + str(args.project_path) + '/images/* ' + str(
             #   args.project_path) + '/images_2/')  # extractor_images files to flir_output folder
            
            os.system('mv ' + str(args.project_path) + '/images/* ' + str(
                args.project_path) + '/flir_output')  # extractor_images files to flir_output folder


            #os.system('mkdir ' + str(args.project_path) + '/backup_2/');
            #os.system('cp ' + str(args.project_path) + '/images/* ' + str(
            #    args.project_path) + '/backup_2/')  # extractor_images files to flir_output folder
            

            os.system('mv ' + str(args.project_path) + '/images_backup/* ' + str(
                args.project_path) + '/images')  # images_raw files to images folder

            #os.system('cp ' + os.path.join(tree.opensfm, "undistorted", "images") + ' ' + os.path.join(tree.opensfm, "undistorted",
            #                                                                            "images_backup"))  # extractor_undistored_images files to images_backup folder
            
            os.rename(os.path.join(tree.opensfm, "undistorted", "images"), os.path.join(tree.opensfm, "undistorted",
                                                                                        "images_backup"))  # extractor_undistored_images files to images_backup folder
            os.system('rm ' + os.path.join(tree.opensfm, "undistorted", "nominal_done.txt"))
            octx.convert_and_undistort(self.rerun(), undistort_callback,
                                       image_list_override)  # undistort das images_raw

            # os.system('mkdir ' + str(args.project_path) + '/images_3/');
            # os.system('cp ' + str(args.project_path) + '/images/* ' + str(
            #     args.project_path) + '/images_3/')  # extractor_images files to flir_output folder
